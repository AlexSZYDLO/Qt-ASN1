...
namespace Value {
  ...
  void ASN1_ValueReal::HexToReal(const ByteArray& input, double& output, std::string& error) {
    ...
    case 0x40:	/* X.690: 8.5.6 a) => 8.5.9 */
      if (octv == 0x40) output = INFINITY; /* 01000000: PLUS-INFINITY */
      else if (octv == 0x41) output = -INFINITY; /* 01000001: MINUS-INFINITY */
      else if (octv == 0x42) output = NAN; /* 01000010: NOT-A-NUMBER */
      else if (octv == 0x43) output = -0.0; /* 01000011: minus zero */
      else error = "Double special value not recognized";
      return;
