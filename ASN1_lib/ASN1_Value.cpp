...
namespace Value {
  ...
  ASN1_Value::ASN1_Value(const std::string& name,
    ...
    m_Tag(tag),
    m_Value(""),
    m_IgnoredByUser(false) { /*std::cout << "ctor: ASN1_Value " << GetName() << " : " << ++static_ObjectCount << std::endl;*/
  }
  ...
  void ASN1_Value::ReadFromBuffer(const ByteArray& buffer, unsigned int& pos) {
    ...
    if (CheckTags(ImplTag, ExplTag, false, 0)) {
      ...
      if (IsMandatory() && !HasDefaultValue()) {
        try { CheckTags(ImplTag, ExplTag, true, oldPos); } // send oldPos because tag is already read, but if the check fails we want the initial position
        catch (ParsingEx& e) {
  ...
  std::string ASN1_Value::StringExtractForLowLevel() const {
    ...
    if (GetType() == cChoice) {
      ASN1_ValueChoice* ChoiceValue = (ASN1_ValueChoice*)(this);
      if (ChoiceValue->GetSelectedChoice() != nullptr) {
        ...
        s << " [ " << "Chosen value:" << " Tag " << ChoiceValue->GetSelectedChoice()->GetTag().GetString() << " - " << "Name " << choiceValueName << " ]";
      }
    }
    else if (GetType() == cSequenceOf) {
      ASN1_ValueSequenceOf* SeqOfValue = (ASN1_ValueSequenceOf*)(this);
  ...
  std::string ASN1_Value::StringExtractForBusinessLevel() const {
    ...
    if (GetType() == cChoice) {
      ...
      if (ChoiceValue->GetSelectedChoice() != nullptr) {
        std::string choiceValueName;
        choiceValueName = ChoiceValue->GetSelectedChoice()->GetName();
        s << " [ " << "Chosen value: " << choiceValueName << " [Tag " << ChoiceValue->GetSelectedChoice()->GetTag().GetString() << "]" << " ] ";
      }
    }
    return s.str();
  }
