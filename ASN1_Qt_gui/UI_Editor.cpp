...
namespace UI {
  ...
  void UI_Editor::SetGrammar(ASN1_Object* obj) {
    m_Grammar = obj;
...
// ---------------------- ACTIONS -------------------
  ...
  void UI_Editor::action_WriteAll() {
    if (m_Grammar) Write(m_Grammar);
  }

  void UI_Editor::action_ReadAll() {
    if (m_Grammar) Read(m_Grammar);
  }
  ...
  void UI_Editor::action_Compare() {
    ...
    QScriptEngine* newEng = InitEngine();
    ASN1_Object* secondTree = nullptr;
