...
namespace UI {
  void UI_ASN1_Node_UTCTime::MakeWidget(QGroupBox* inBox, bool readOnly) {
    ...
    QDate date(y + 2000, m, d); QTime time(h, min, s);
  ...
  bool UI_ASN1_Node_UTCTime::accept(ASN1_Object* val) {
    ...
    Utils::ValuesToUTCTime(edit->date().year() - 2000, edit->date().month(), edit->date().day(),
                           edit->time().hour(), edit->time().minute(), edit->time().second(),
                           stringToSign(ZSign->currentText()), Zhours->value(), ZMin->value(), UTCTimeStringTmp, errorBuffer);
