...
void Generator::SetLastVariableName(const std::string &newName)
{
  if (!currentVariableList->at(currentVariableList->size() - 1).unknownType)
    log(string("variable: " + Utils::NodeTypeToString(currentVariableList->at(currentVariableList->size() - 1).type)
               + " - name: " + newName));
  else
...
string *Generator::AddUniqueModule(eNodeType type)
{
  ...
  m_ModuleList.push_back({NodeTypeToString(type) + to_string(UniqueModuleCount),
                          type,
                          *currentVariableList,
                          m_ExtensibilityImplied || tempExtensibility});
