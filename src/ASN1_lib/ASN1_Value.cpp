...
ASN1_Object *ASN1_Value::GetGrammarObject() const
{
  return m_GrammarObject;
}
const ASN1_Value *ASN1_Value::GetMyOwner() const
{
  return m_ParentValue;
}
const ASN1_Value *ASN1_Value::GetDefaultValue() const
{
  return m_DefaultValue;
}
const ByteArray ASN1_Value::GetTag() const
{
  return m_Tag;
}
const ByteArray ASN1_Value::GetDefaultTag() const
{
  return m_DefaultTag;
}
std::string ASN1_Value::GetName() const
{
  return m_Name;
}
std::string ASN1_Value::GetDescription() const
{
  return m_Description;
}
unsigned int ASN1_Value::GetLastLength() const
{
  return m_TmpLength;
}
bool ASN1_Value::HasDefaultValue() const
{
  return m_DefaultValue != nullptr;
}
bool ASN1_Value::IsMandatory() const
{
  return !m_Optional;
}
bool ASN1_Value::IsExplicit() const
{
  return m_Explicit;
}

const ByteArray ASN1_Value::GetValue() const
{
  return m_Value;
}
bool ASN1_Value::IsIgnored() const
{
  return m_IgnoredByUser;
}
...
bool ASN1_Value::ReadNextTags(const ByteArray &buffer, unsigned int &pos, ByteArray &ImplTag, ByteArray &ExplTag) const
{
  try
  {
    ...
    if (IsExplicit() && m_Tag != "" && m_DefaultTag != "")
    {
      if (!GetLengthFromBuffer(buffer, pos, dummyL))
        throw ParsingEx(
            "Cannot read length between implicit and explicit tag. No data to read.", this->GetGrammarObject(), pos);
  ...
  catch (ParsingEx &e)
  {
    ...
    { // buffer empty but value is optional (it will be handled later)
...
bool ASN1_Value::CheckTags(const ByteArray &ImplTag, const ByteArray &ExplTag, bool throwEx, unsigned int pos) const
{
  ...
  if (m_Tag != "")
  {
    ...
    {
      if (throwEx)
        throw ParsingEx("First tag check failed: read " + ImplTag.GetString() + ", expecting " + m_Tag.GetString(),
                        this->GetGrammarObject(),
                        pos);
      return false;
    }
  }
  else if (m_DefaultTag != "")
  {
    if (m_DefaultTag != ImplTag)
    {
      if (throwEx)
        throw ParsingEx(
            "First tag check failed: read " + ImplTag.GetString() + ", expecting " + m_DefaultTag.GetString(),
            this->GetGrammarObject(),
            pos);
      return false;
    }
  ...
  if (IsExplicit())
  {
    if (m_DefaultTag != "" && m_DefaultTag != ExplTag)
    {
      if (throwEx)
        throw ParsingEx(
            "Second tag check failed: read " + ImplTag.GetString() + ", expecting " + m_DefaultTag.GetString(),
            this->GetGrammarObject(),
            pos);
      return false;
    }
  ...
  if (GetType() == SecondTree->GetType())
  {
    if (!IsMandatory())
      if (IsIgnored() != SecondTree->IsIgnored())
      {
        ...
        errorReport.append("Tree Node: " + std::to_string(IsIgnored())
                           + " - Compared Node: " + std::to_string(SecondTree->IsIgnored()) + "\n\n");
        NbDiff++;
      }
