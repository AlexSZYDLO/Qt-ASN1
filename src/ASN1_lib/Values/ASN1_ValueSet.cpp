  ...
  if (CheckTags(ImplTag, ExplTag, false, 0))
  {
      ...
      for (unsigned int i = 0; i < m_Objects.size(); i++)
      {
        ...
        if (Object->CheckTags(ImplTag, ExplTag, false, 0))
        {
          ...
          if (found)
          {
            ...
            throw ParsingEx(
                "The object has already been processed in set: set" + GetName() + ", object " + Object->GetName(),
    ...
    for (unsigned int i = 0; i < MyObjectsToFill.size(); i++)
    {
      ...
      if (Object->IsMandatory() || (Object->IsMandatory() && !Object->HasDefaultValue()))
        throw ParsingEx("Missing mandatory object in set: " + GetName() + ", object " + Object->GetName(),
                        this->GetGrammarObject(),
                        oldPos);
      else
        Object->Ignore(true);
