  ...
  if (!choiceIgnored)
  {
    ...
    if (!ObjectFound)
    {
      if (IsMandatory() && !HasDefaultValue())
      {
        ...
        {
          e.AddError("Cannot find a matching tag in choice: " + GetName() + ". Read tags are: first tag "
                     + ImplTag.GetString() + ", second tag (to be ignored for implicit objects) " + ExplTag.GetString());
          throw e;
        }
      }
      else
...
bool ASN1_ValueChoice::CompareTree(const ASN1_Value *SecondTree, unsigned int &NbDiff, std::string &errorReport) const
{
  ...
  if (result && !IsIgnored())
  {
    if ((SecondTreeAsChoice->GetSelectedChoice() != nullptr)
        && (m_TheChoice->GetTag() == SecondTreeAsChoice->GetSelectedChoice()->GetTag()))
    ...
    else
    {
      ...
      errorReport.append("Tree Node: " + GetSelectedChoice()->GetName()
                         + " - Compared Node: " + SecondTreeAsChoice->GetSelectedChoice()->GetName() + "\n\n");
      NbDiff++;
    }
  }
  return result;
