    ...
    for (std::vector<ASN1_Value *>::iterator i = m_Objects.begin(); i < m_Objects.end(); i++)
    {
      ...
      else
      {
        ...
        if (!(*i)->IsMandatory() || (*i)->HasDefaultValue())
          ...
          throw ParsingEx(
              "Cannot find mandatory object in " + GetName() + ": " + (*i)->GetName(), this->GetGrammarObject(), pos);
      }
    }
    if (pos < dataFieldPos + L)
    {
      if (IsExtensible())
      {
        ...
        while (pos < dataFieldPos + L)
        {
          ...
          try
          {
            ReadNextTags(buffer, pos, TagExtensibility, TagDummy);
          }
          catch (ParsingEx &e)
          {
        ...
        throw ParsingEx("Sequence too long. There is more to read but the sequence is finished: sequence " + GetName(),
  ...
  if (result && !IsIgnored())
  {
    ...
    if (IsExtensible())
    {
      if (GetExtensibilitySize() == SecondTreeAsSeq->GetExtensibilitySize())
        ...
        {
          unsigned int oldNbDiff = NbDiff;
          result = GetExtensibilityObjectAt(i)->CompareTree(
              SecondTreeAsSeq->GetExtensibilityObjectAt(i), NbDiff, errorReport);
          if (oldNbDiff != NbDiff)
          {
            errorReport.append("Node: " + GetName() + " - Object comparison failed in extensibility at index "
                               + std::to_string(i) + ".\n\n");
            NbDiff++;
          }
          if (!result)
