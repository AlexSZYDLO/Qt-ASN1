...
namespace ASN1_DLL {
  ...
  typedef bool (*tByteArray_GetBytesAtRank)(const DLL_ByteArray* hex, unsigned int firstByteRank, unsigned int nbOfBytes, DLL_ByteArray* out);
  ...
  typedef void (*tUtils_MakeTag)(unsigned int tagClass, unsigned int tagForm, const DLL_ByteArray* label, DLL_ByteArray* out);
  typedef bool (*tUtils_DecomposeTag)(const DLL_ByteArray* tag, unsigned int* tagClass, unsigned int* tagForm, DLL_ByteArray* label);
  ...
  typedef void (*tUtils_UTCTimeToValues)(const char* input, int* aYear_YY, int* aMonth_MM, int* aDay_DD, int* aHours_HH, int* aMinutes_MM, int* aSeconds_SS,
                                         unsigned int* aZone, int* aZoneHours_HH, int* aZoneMinutes_MM, char* errorBuffer, unsigned int errorBufferSize);
  typedef void (*tUtils_ValuesToUTCTime)(int aYear_YY, int aMonth_MM, int aDay_DD, int aHours_HH, int aMinutes_MM, int aSeconds_SS,
                                         unsigned int aZone, int aZoneHours_HH, int aZoneMinutes_MM,
                                         char* output, unsigned int outputBufferSize, char* errorBuffer, unsigned int errorBufferSize);
  ...
  typedef bool (*tASN1_Object_ReadFromBuffer)(const DLL_ASN1_Object* obj, const DLL_ByteArray* buffer,
                                              char* error, unsigned int errorBufferSize);
  typedef bool (*tASN1_Object_Compare)(const DLL_ASN1_Object* obj, const DLL_ASN1_Object* secondTree,
...
#define GET_FUNC_POINTER(name) \
  name = (t##name)GetProcAddress(hDLL, #name);
#endif

#ifndef TYPEDEF_WITH_BUFFER
#define TYPEDEF_WITH_BUFFER(name)                                                                                           \
  typedef void (*tASN1_##name##_Get##name##Value)(const DLL_ASN1_##name* obj, char* output, unsigned int outputBufferSize); \
  typedef void (*tASN1_##name##_HexTo##name)(const DLL_ByteArray* input, char* output, unsigned int outputBufferSize, char* errorBuffer, unsigned int errorBufferSize);
#endif
...
#define TYPEDEF_WITH_TYPE(name, type)                                                        \
  ...
  typedef void (*tASN1_##name##_HexTo##name)(const DLL_ByteArray* input, type* output, char* errorBuffer, unsigned int errorBufferSize);
#endif
...
#define TYPEDEF_COMMON_FUNCTIONS(name, type)                                                                                                    \
  ...
  typedef DLL_ASN1_##name* (*tASN1_##name##_New)(const char* name, const DLL_ByteArray* tag, bool optional,                                     \
                                                 bool explicitTag, const DLL_ASN1_##name* defaultValue);                                        \
  typedef void (*tASN1_##name##_Set##name##Value)(DLL_ASN1_##name * obj, const type val);                                                       \
  typedef void (*tASN1_##name##_Set##name##ValueCheck)(DLL_ASN1_##name * obj, const type val, char* errorBuffer, unsigned int errorBufferSize); \
  typedef void (*tASN1_##name##_##name##ToHex)(const type input, DLL_ByteArray* output, char* errorBuffer, unsigned int errorBufferSize);
#endif
...
#define DLL_FUNCTION_POINTER(name)                                          \
  ...
  bool Check_##name() { return (ASN1_##name##_New && ASN1_##name##_Get##name##Value && ASN1_##name##_Set##name##Value && ASN1_##name##_Set##name##ValueCheck && ASN1_##name##_##name##ToHex && ASN1_##name##_HexTo##name); }
#endif
...
#define DLL_NODE_TYPE(name, type)      \
  ...
  typedef DLL_ASN1_Choice* (*tASN1_Choice_New)(DLL_callbackChoice choiceFromIdx, unsigned int maxChoiceIdx, const char* name, const DLL_ByteArray* tag,
                                               bool optional, bool explicitTag, const DLL_ASN1_Choice* defaultValue);
  ...
  // ------------ ASN1_SEQUENCE ------------
  typedef DLL_ASN1_Sequence* (*tASN1_Sequence_New)(DLL_ASN1_Object* valuesArray[], unsigned int numberOfValues, const char* name, const DLL_ByteArray* tag,
                                                   bool optional, bool explicitTag, const DLL_ASN1_Sequence* defaultValue, bool extensibility);
  ...
  typedef DLL_ASN1_SequenceOf* (*tASN1_SequenceOf_New)(DLL_callbackSequenceOf newSeqOfObject, const char* name, const DLL_ByteArray* tag,
                                                       bool optional, bool explicitTag, const DLL_ASN1_SequenceOf* defaultValue);
  ...
  // ------------ ASN1_SET ------------
  typedef DLL_ASN1_Set* (*tASN1_Set_New)(DLL_ASN1_Object* valuesArray[], unsigned int numberOfValues, const char* name, const DLL_ByteArray* tag,
                                         bool optional, bool explicitTag, const DLL_ASN1_Set* defaultValue, bool extensibility);
  ...
  bool LoadASN1DLL(const std::wstring& dllPath) {
    ...
    if(!ByteArray_New || !ByteArray_NewFromByteArray || !ByteArray_Delete || !ByteArray_Size || !ByteArray_Equals || !ByteArray_GetString || !ByteArray_GetByteAtRank || !ByteArray_GetBytesAtRank || !ByteArray_Clear || !ByteArray_Append || !ByteArray_InsertAt) return false;
    if(!Utils_IntAsHex || !Utils_HexAsInt || !Utils_BinaryAsHex || !Utils_HexAsBinary || !Utils_HexAsBinary8Padded || !Utils_IntAsBinary || !Utils_IntAsBinary8Padded || !Utils_BinaryAsInt || !Utils_IsHex || !Utils_IsBinary || !Utils_IsComplexType || !Utils_GetTagFromBuffer || !Utils_GetLengthFromBuffer || !Utils_MakeLength || !Utils_MakeTag || !Utils_DecomposeTag || !Utils_StringToHex || !Utils_HexToString || !Utils_UTCTimeToValues || !Utils_ValuesToUTCTime || !Utils_IsValidObjectID || !Utils_IsValidUTCTime || !Utils_IsValidIA5String || !Utils_IsValidUTF8String) return false;
    if(!ASN1_Object_GetMyOwner || !ASN1_Object_GetDefaultValue || !ASN1_Object_GetDefaultTag || !ASN1_Object_GetTag || !ASN1_Object_GetDescription || !ASN1_Object_StringExtract || !ASN1_Object_StringExtractForResearch || !ASN1_Object_GetType || !ASN1_Object_IsMandatory || !ASN1_Object_IsIgnored || !ASN1_Object_IsExplicit || !ASN1_Object_GetHexBuffer || !ASN1_Object_GetLastSize || !ASN1_Object_ClearDynamicData || !ASN1_Object_SetDescription || !ASN1_Object_Ignore || !ASN1_Object_WriteIntoBuffer || !ASN1_Object_ReadFromBuffer || !ASN1_Object_Compare || !ASN1_Object_Delete) return false;
    if(!ASN1_Choice_New || !ASN1_Choice_AvailableChoices || !ASN1_Choice_GetChoiceFromIndex || !ASN1_Choice_MakeDummyChoiceList || !ASN1_Choice_DeleteDummyChoiceList || !ASN1_Choice_SetSelectedChoice || !ASN1_Choice_GetSelectedChoice || !ASN1_Choice_GetSelectedChoiceIndex) return false;
    if(!ASN1_Sequence_New || !ASN1_Sequence_GetObjectAt || !ASN1_Sequence_GetSize || !ASN1_Sequence_GetExtensibilityObjectAt || !ASN1_Sequence_GetExtensibilitySize || !ASN1_Sequence_IsExtensible) return false;
    if(!ASN1_SequenceOf_New || !ASN1_SequenceOf_GetObjectAt || !ASN1_SequenceOf_GetSize || !ASN1_SequenceOf_AppendNewObject || !ASN1_SequenceOf_DeleteObjectAt || !ASN1_SequenceOf_MoveUpObject || !ASN1_SequenceOf_MoveDownObject || !ASN1_SequenceOf_GetDummyNewObject) return false;
    if(!ASN1_Set_New || !ASN1_Set_GetObjectAt || !ASN1_Set_GetSize || !ASN1_Set_GetExtensibilityObjectAt || !ASN1_Set_GetExtensibilitySize || !ASN1_Set_IsExtensible) return false;
    if(!Check_BitString() || !Check_Boolean() || !Check_Enumerated() || !Check_IA5String() || !Check_Integer() || !Check_Null() || !Check_OctetString() || !Check_ObjectID() || !Check_Real() || !Check_UTCTime() || !Check_UTF8String()) return false;
  ...
  class Hex {
   ...
   public:
    Hex(const char* str) { ptr = ByteArray_New(str); }
    Hex(DLL_ByteArray* ptr2) { ptr = ByteArray_NewFromByteArray(ptr2); }
    ~Hex() { ByteArray_Delete(ptr); }
    DLL_ByteArray* p() { return ptr; }
  };
