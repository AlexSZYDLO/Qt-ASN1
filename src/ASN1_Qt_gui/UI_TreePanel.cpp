...
namespace UI {
  ...
  ASN1_Item* UI_TreePanel::GetNextNode(ASN1_Item* OfNode) const {
    ...
    if (result == nullptr) result = m_Model->itemFromIndex(node.parent().model()->index(node.row() + 1, 0));
  ...
  ASN1_Item* UI_TreePanel::GetPrevNode(ASN1_Item* OfNode) const {
    ...
    result = m_Model->itemFromIndex(node.parent().model()->index(node.row() - 1, 0));
    if (result == nullptr) result = m_Model->itemFromIndex(node.parent().model()->index(node.row(), 0));
    return static_cast<ASN1_Item*>(result);
  }
