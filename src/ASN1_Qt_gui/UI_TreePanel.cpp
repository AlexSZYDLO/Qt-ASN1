...
UI_TreePanel::UI_TreePanel(UI_Editor *parent, std::shared_ptr<ASN1_Object> grammar = nullptr)
    : m_Parent(parent)
    , m_Model(nullptr)
    , m_Grammar(grammar)
    , m_SearchPattern("")
...
QMenu *UI_TreePanel::MakeContextMenu(const ASN1_Item *node) const
{
  ...
  if (node->getGrammarObject() != nullptr)
  {
    ...
    {
      case cBitString:
        contextMenu->addAction("Bit String Converter", m_Parent, SLOT(action_ConvertBitString()));
        break;
      case cBoolean:
        contextMenu->addAction("Boolean Converter", m_Parent, SLOT(action_ConvertBoolean()));
        break;
      case cEnumerated:
        contextMenu->addAction("Enumerated Converter", m_Parent, SLOT(action_ConvertEnumerated()));
        break;
      case cInteger:
        contextMenu->addAction("Integer Converter", m_Parent, SLOT(action_ConvertInteger()));
        break;
      case cIA5String:
        contextMenu->addAction("IA5 String Converter", m_Parent, SLOT(action_ConvertIA5String()));
        break;
      case cObjectID:
        contextMenu->addAction("Object ID Converter", m_Parent, SLOT(action_ConvertObjectID()));
        break;
      case cOctetString:
        contextMenu->addAction("Octet String Converter", m_Parent, SLOT(action_ConvertOctetString()));
        break;
      case cReal:
        contextMenu->addAction("Real Converter", m_Parent, SLOT(action_ConvertReal()));
        break;
      case cUTCTime:
        contextMenu->addAction("UTC Time Converter", m_Parent, SLOT(action_ConvertUTCTime()));
        break;
      case cUTF8String:
        contextMenu->addAction("UTF8 String Converter", m_Parent, SLOT(action_ConvertUTF8String()));
        break;
      case cChoice:
        contextMenu->addAction("Select Choice", this, SLOT(QuickActionChoiceSelect()));
        break;
      case cSequenceOf:
        contextMenu->addAction("Add Object", this, SLOT(QuickActionSeqOfAddObject()));
        break;
      default:
        break;
    }
    ...
    contextMenu->addSeparator();
    if (node->getGrammarObject()->GetMyOwner() != nullptr
        && node->getGrammarObject()->GetMyOwner()->GetType() == cSequenceOf)
    {
...
ASN1_Item *UI_TreePanel::RefreshBranch(ASN1_Item *fromItem)
{
  ...
  QObject::disconnect(
      treeSel, SIGNAL(currentChanged(QModelIndex, QModelIndex)), this, SLOT(RefreshQuickActions(QModelIndex)));
...
void UI_TreePanel::RefreshQuickActions(QModelIndex idx)
{
  ...
  {
    ClickableLabel *selectChoice = MakeClickableLabel(
        "commonButton", "Select choice", p + "select.png", m_UIQuickActionsBox, QSize(25, 25));
  ...
  {
    ClickableLabel *addObject = MakeClickableLabel(
        "commonButton", "Add Object", p + "plus.png", m_UIQuickActionsBox, QSize(25, 25));
  ...
  {
    ClickableLabel *moveUp = MakeClickableLabel(
        "commonButton", "Move Up", p + "up.png", m_UIQuickActionsBox, QSize(25, 25));
    ClickableLabel *moveDown = MakeClickableLabel(
        "commonButton", "Move Down", p + "down.png", m_UIQuickActionsBox, QSize(25, 25));
    ClickableLabel *deleteObj = MakeClickableLabel(
        "commonButton", "Delete", p + "minus.png", m_UIQuickActionsBox, QSize(25, 25));
...
void UI_TreePanel::LoadGrammar(std::shared_ptr<ASN1_Object> newGrammar)
{
  ASN1_ItemModel *prevmodel = m_Model;
  m_Model = new ASN1_ItemModel(newGrammar.get(), m_ShowChoices);
...
ASN1_Item *UI_TreePanel::GetNextNode(ASN1_Item *OfNode) const
{
  ...
  // try to get a child
  // QStandardItem* result = m_Model->itemFromIndex(node.index(0, 0));
  QStandardItem *result = m_Model->itemFromIndex(node.model()->index(0, 0));

  // try to get next node on same level
  if (result == nullptr)
    result = m_Model->itemFromIndex(node.parent().model()->index(node.row() + 1, 0));
...
ASN1_Item *UI_TreePanel::GetPrevNode(ASN1_Item *OfNode) const
{
  ...
  result = m_Model->itemFromIndex(node.parent().model()->index(node.row() - 1, 0));
  if (result == nullptr)
    result = m_Model->itemFromIndex(node.parent().model()->index(node.row(), 0));
  return static_cast<ASN1_Item *>(result);
}
...
QWidget *UI_TreePanel::CreateViewOptionsBox(QWidget *parent)
{
  ...
  QString p = ":/img/icon_set_2/";
  ClickableLabel *collapseAll = MakeClickableLabel(
      "commonButton", "Collapse Tree", p + "collapse.png", box, QSize(25, 25));
