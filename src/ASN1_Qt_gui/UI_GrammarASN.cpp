...
UI_GrammarASN::UI_GrammarASN(UI_Main *main)
    : QWidget(main)
    , m_Main(main)
{
  MakeASNGrammarPane();
...
QWidget *UI_GrammarASN::MakeMenuBar()
{
  ...
  ClickableLabel *buttonToJS = MakeClickableLabel(
      "actionButton", "Process ASN grammar to JavaScript", p + "js.png", this);
...
void MakeASNHintList(QStandardItemModel *model)
{
  ...
  // Root
  subList = {new DraggableStandardItem("DEFINITIONS", "Keyword to put at the beginning of the module definitions.", ""),
  ...
  subList = {new DraggableStandardItem("Sequence Template", "Template for Sequence module creation.", t1, t1),
  ...
  current = new DraggableStandardItem(
      "Node Options", "Option keywords applicable to the nodes.", "Not draggable", "", false);
  current->appendColumn(subList);
  itemList.push_back(current);
  subList.clear();
...
void UI_GrammarASN::ToEditor()
{
  ...
  if (!js.isEmpty())
  {
    std::shared_ptr<ASN1_Object> o = m_Main->JSToObj(m_Main->GetScriptEngine(), js, false);
...
{
  QString fileName = QFileDialog::getOpenFileName(this, "Open ASN File", "../Grammar", "ASN file (*.asn *)");
  if (fileName != "")
  {
    QFile f(fileName);
    if (f.exists())
...
void UI_GrammarASN::SaveASN()
{
  ...
  QString fileName = QFileDialog::getSaveFileName(this, "Save ASN File", defaultFile, "ASN file (*.asn *)");
  QFile f(fileName);
  if (f.open(QIODevice::WriteOnly))
