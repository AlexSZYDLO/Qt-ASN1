...
void UI_ASN1_Node_OctetString::MakeWidget(QGroupBox *inBox, bool readOnly)
{
  ...
  hexaText->setPlainText(m_Grammar->GetOctetStringValue().GetString());
  if (readOnly)
...
bool UI_ASN1_Node_OctetString::accept(ASN1_Object *val)
{
  ...
  {
    octetStringVal->SetOctetStringValue(hex.toStdString().c_str());
    return true;
  }
  return false;
}
...
bool UI_ASN1_Node_OctetString::GetHexValue(ASN1_Object *val, ByteArray &hex)
{
  ...
  QString temp(m_Grammar->GetOctetStringValue().GetString());
  octetStringVal->SetOctetStringValue(hexaText->toTextWithoutSpace().toStdString().c_str());
  octetStringVal->WriteIntoBuffer(hex);
  octetStringVal->SetOctetStringValue(temp.toStdString().c_str());
  return true;
}
} // namespace UI
