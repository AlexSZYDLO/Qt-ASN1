...
void MakeOptionsLayout(QGroupBox *groupBox_Grammar, QVBoxLayout *vLayGrammarLayout, const ASN1_Object *val)
{
  ...
  if (val->IsMandatory())
    label_Optional->setText("Mandatory");
  else
    label_Optional->setText("Optional");

  if (val->IsExplicit())
    label_TaggingConvention->setText("Explicit tagging");
  else
    label_TaggingConvention->setText("Implicit tagging");
...
void MakeLinkValuesLayout(QGroupBox *groupBox_Grammar,
                          QVBoxLayout *vLayGrammarLayout,
                          const ASN1_Object *val,
                          QWidget *sigConnection)
{
    ...
    , value(val)
{
  ...
  lineEdit_Description->setToolTip(
      "Description is only a comment, it is not part of ASN1 standard, "
