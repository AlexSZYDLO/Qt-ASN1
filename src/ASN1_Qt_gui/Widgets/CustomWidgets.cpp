...
{
  if (type == cLineEditBinary)
    return 8;
  if (type == cLineEditHexa)
    return 2;
  else
    return std::numeric_limits<int>::max(); // should be enough not to be reached
}

myQTextEdit::myQTextEdit(QWidget *parent, cLineEditType aType)
    : QPlainTextEdit(parent)
    , filterType(aType)
    , bCheckText(true)
    , bInsertSpaces(true)
    , cPackSize(GetPackSize(aType))
{
...
int myQTextEdit::CalculateCursorLocation(int textSizeWithoutSpaces) const
{
  ...
  spaceCount += (spaceCount != 0 && textSizeWithoutSpaces % cPackSize == 0)
                    ? -1
                    : 0; // remove 1 if even since we must not count the space at end
  ...
  if (filterType == cLineEditHexa)
  {
    if (toTextWithoutSpace().size() % 2 != 0)
    {
      if (QMessageBox::question(nullptr,
                                ...
                                "The value must be a sequence of 8 bits octets."
...
{
  if (filterType == cLineEditBinary || filterType == cLineEditNumeric || filterType == cLineEditHexa
      || filterType == cLineEditOID)
  {
    QRegularExpression regEx;
    if (filterType == cLineEditBinary)
      regEx.setPattern("^[01]*$");
    if (filterType == cLineEditNumeric)
      regEx.setPattern("^[0-9]*$");
    if (filterType == cLineEditHexa)
      regEx.setPattern("^[0-9a-fA-F]*$");
    if (filterType == cLineEditOID)
      regEx.setPattern("^[0-9.]*$");
...
bool ClickableLabel::IsPressed() const
{
  return pressed;
}
void ClickableLabel::SetPressed(bool _pressed)
{
  pressed = _pressed;
}
...
ClickableLabel *
MakeClickableLabel(const QString &name, const QString &tooltip, const QString &icon, QWidget *parent, QSize size)
{
