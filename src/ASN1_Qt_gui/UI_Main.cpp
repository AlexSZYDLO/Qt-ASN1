...
namespace UI {
  ...
  void UI_Main::SetEditorObj(std::shared_ptr<ASN1_Object> obj) {
  ...
  std::shared_ptr<ASN1_Object> UI_Main::JSToObj(QJSEngine** eng, const QString& js, bool debug, bool silent) {
    QString err;
    ...
    *eng = InitEngine();
    ...
    QApplication::setOverrideCursor(Qt::WaitCursor);
    std::shared_ptr<ASN1_Object> obj = GrammarFromScript(*eng, js, err);
    QApplication::restoreOverrideCursor();
  ...
  bool UI_Main::ProcessAdditionalJS(QJSEngine** eng, const QString& js, bool debug) {
    QString error;
    if (*eng == nullptr) {
  ...
  std::shared_ptr<ASN1_Object> UI_Main::CompToObj(const QString& name, bool silent) {
    std::shared_ptr<ASN1_Object> obj = nullptr;
    if (name == "Test_1") {
      Grammar::SequenceTestVars s;
      obj.reset(Grammar::MakeSequenceTest("Test_Grammar", "", s));
    }
    else if (name == "SIM Alliance") {
      obj.reset(GetGrammar());
    }
    if (!silent) {
      if (obj != nullptr)
