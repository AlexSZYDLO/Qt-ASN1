...
ASN1_Script_Choice::ASN1_Script_Choice(QJSValue callback, int numberOfChoices,
                                       const QString& name, const QString& tag,
                                       bool optional, bool explicitTag,
                                       ASN1_Script_Choice* defaultValue)
    : ASN1_Script_Node_Base(name), m_Callback(callback) {
  ASN1_Choice* def = (defaultValue && !defaultValue->used) ? static_cast<ASN1_Choice*>(defaultValue->getGrammar()) : nullptr;
  ...
  m_GrammarSpe = new ASN1_Choice({_callback, this}, numberOfChoices, name.toStdString().c_str(),
                    ByteArray(tag.toStdString().c_str()), optional, explicitTag, def);
