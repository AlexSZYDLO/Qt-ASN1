...
ASN1_Script_SequenceOf::ASN1_Script_SequenceOf(QJSValue callback,
                                               const QString &name,
                                               const QString &tag,
                                               bool optional,
                                               bool explicitTag,
    ...
    , m_Callback(callback)
{
  ASN1_SequenceOf *def = (defaultValue && !defaultValue->used)
                             ? static_cast<ASN1_SequenceOf *>(defaultValue->getGrammar())
                             : nullptr;
  if (def)
  ...
  m_GrammarSpe = new ASN1_SequenceOf(
      {_callback, this}, name.toStdString().c_str(), ByteArray(tag.toStdString().c_str()), optional, explicitTag, def);
