...
string CPPGenerator::typeToCPPClass(eNodeType t) {
  switch (t) {
  case cBoolean: return "ASN1_Boolean";
  case cBitString: return "ASN1_BitString";
  case cChoice: return "ASN1_Choice";
  case cEnumerated: return "ASN1_Enumerated";
  case cIA5String: return "ASN1_IA5String";
  case cInteger: return "ASN1_Integer";
  case cNull: return "ASN1_Null";
  case cObjectID: return "ASN1_ObjectID";
  case cOctetString: return "ASN1_OctetString";
  case cReal: return "ASN1_Real";
  case cSequence: return "ASN1_Sequence";
  case cSequenceOf: return "ASN1_SequenceOf";
  case cSet: return "ASN1_Set";
  case cUTCTime: return "ASN1_UTCTime";
  case cUTF8String: return "ASN1_UTF8String";
  default: return "";
  }
}
...
inline string TAB(int i) { string s; for (int j = 0; j < i; j++) s += "\t"; return s; }
...
void CPPGenerator::GenerateVar(Variable& v, int tab) {
  ...
  if (v.type == cSequence || v.type == cSet) {
    ...
    cppCode += "ASN1_Object* " + v_name + " = new " + typeToCPPClass(v.type)
               + "({&" + PAR(callbackName) + PAR("nullptr}") + PARS(v.name) + PARS(v.tag) + PARB(v.optional) + PARB(v.explicitTag) + PAR("nullptr", false) + ");";
  }
    ...
    cppCode += "ASN1_Object* " + v_name + " = new " + typeToCPPClass(v.type)
               + "({&" + PAR(callbackName) + PAR("nullptr}") + PAR(choiceSize) + PARS(v.name) + PARS(v.tag) + PARB(v.optional) + PARB(v.explicitTag) + PAR("nullptr", false) + ");";
  }
  else {
    if (v.customTypeName == "") {
      cppCode += "ASN1_Object* " + v_name + " = new " + typeToCPPClass(v.type)
                 + "(" + PARS(v.name) + PARS(v.tag) + PARB(v.optional) + PARB(v.explicitTag) + PAR("nullptr", false) + ");";
    }
    else {
      cppCode += "ASN1_Object* " + v_name + " = Make_" + v.customTypeName
                 + "(" + PARS(v.name) + PARS(v.tag) + PARB(v.optional) + PARB(v.explicitTag) + PAR("nullptr", false) + ");";
    }
  }
  cppCode += "\n";
